...
> 更新日期：2025-11-30 19:40（修正兆豐銀行下載連結選擇器，解析已正常）
...
| 26 | 三信商業銀行 | ✅ | ❌ 0/8 | p.28 | ✅ | ✅ 8/8 | p.3 | 已輸出 xlsx (僅114Q1)，113Q4 為資本適足性報告非財報 |
| 27 | 聯邦商業銀行 | ✅ | ❌ 0/8 | - | ✅ | ✅ 8/8 | p.11 | 已輸出 xlsx (僅114Q1)，113Q4 PDF Pattern 錯誤無法解析 |
| 28 | 遠東國際商業銀行 | ✅ | ❌ 0/8 | p.3 | ✅ | ❌ 0/8 | p.3 | PDF 文字被拆散成單字垂直排列，數據存在但需專用解析器 |
...
**問題分類**：
- ❌ 無法修復（PDF 本身無資料）：23, 25, 26, 27, 29, 33 的 113Q4
- ⚠️ 需 OCR：20 的 114Q1, 21 的 113Q4/114Q1
- ⚠️ 需專用解析器：07 的 114Q1, 28 的 113Q4/114Q1, 37 的 113Q4/114Q1
...
| 26 | 三信商業銀行 | PDF 內容問題 | PDF 是資本適足性報告，非財報 |
| 27 | 聯邦商業銀行 | PDF Pattern 錯誤 | pdfplumber 無法正確讀取，Pattern 錯誤 |
| 28 | 遠東國際商業銀行 | 特殊格式 | 文字被拆散成單字垂直排列 |
...
| 28 | 遠東國際商業銀行 | 特殊格式 | 同 113Q4 |
| 37 | 中國信託商業銀行 | 特殊格式 | 垂直文字表格排版 |

**問題分類統計**：
- PDF 內容問題（5 項）：PDF 不含資產品質資料，無法透過程式修復
- 掃描式 PDF（3 項）：需安裝 Tesseract OCR 才能處理
- 特殊格式（4 項）：需開發專用解析器處理特殊排版

**結論**：
- 約 42% 的錯誤是因為 PDF 本身不包含所需資料，需向銀行確認是否有其他來源
- 約 25% 需要 OCR 功能（需安裝 tesseract + pytesseract + pdf2image）
- 約 33% 需要開發專用解析器
