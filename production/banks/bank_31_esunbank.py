...
class ESunBankDownloader(BaseBankDownloader):
    ...
    async def _download(self, page: Page, year: int, quarter: int) -> DownloadResult:
        ...
        rows = await tbodies[1].query_selector_all("tr")
        
        file_name = None
        for row in rows[1:]:  # 跳過標題列
            tds = await row.query_selector_all("td")
            if len(tds) >= 8:
                text_quarter = await tds[1].inner_text()
                report_type = await tds[5].inner_text()
                
                # 根據季度選擇合併或個體財報
                if quarter % 2 == 1:
                    if quarter_text in text_quarter and report_type == "IFRSs合併財報":
                        file_name = await tds[7].inner_text()
                        break
                else:
        ...
        # 取得實際 PDF 連結
        pdf_link = await page.query_selector("a")
        if not pdf_link:
            return DownloadResult(
                status=DownloadStatus.NO_DATA,
                message="找不到 PDF 連結"
